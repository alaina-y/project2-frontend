...
import { getAutoPolicies, createAutoPolicy, updateAutoPolicy, deleteAutoPolicy, getCustomers } from '../../services/api';
import { normalizeArray } from '../../utils/normalizeArray';

const emptyForm = { cust_id: '', start_date: '', end_date: '', premium_amount: '', a_policy_status: 'C' };
...
export default function AutoPolicyManagement() {
  ...
  const reload = async () => {
    console.log('AutoPolicyManagement mounted');
    try {
      const [polRes, custRes] = await Promise.all([getAutoPolicies(), getCustomers()]);
      console.log('API response (auto policies):', polRes);
      console.log('API response (customers):', custRes);
      setData(normalizeArray(polRes, 'auto_policies'));
  ...
  const filtered = data.filter((p) => {
    ...
    return String(p.a_policy_id).includes(search) || name.toLowerCase().includes(search.toLowerCase());
  });

  const openAdd  = () => { setEditing(null); setForm(emptyForm); setModalOpen(true); };
  const openEdit = (row) => {
    setEditing(row);
    setForm({ cust_id: String(row.cust_id), start_date: row.start_date, end_date: row.end_date, premium_amount: String(row.premium_amount), a_policy_status: row.a_policy_status });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await updateAutoPolicy(editing.a_policy_id, payload);
      } else {
        await createAutoPolicy(payload);
      }
      await reload();
      handleClose();
    } catch (err) {
  ...
  const confirmDelete = async () => {
    try {
      await deleteAutoPolicy(deleteTarget.a_policy_id);
  ...
  const columns = [
    ...
    { key: 'a_policy_status', label: 'Status',   render: (v) => <StatusBadge status={v} /> },
  ];
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <Header title="Auto Policy Management" />
        <main className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <button onClick={openAdd} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition-colors">+ Add Auto Policy</button>
          </div>
          {loading ? (
            ...
            <>
              ...
              <DataTable columns={columns} data={filtered} keyField="a_policy_id" onEdit={openEdit} onDelete={setDeleteTarget} />
            </>
          )}
        </main>
      </div>

      <FormModal isOpen={modalOpen} onClose={handleClose} title={editing ? 'Edit Auto Policy' : 'Add Auto Policy'} onSubmit={handleSubmit} submitLabel={submitting ? 'Saving…' : editing ? 'Update' : 'Create'}>
        <FieldGroup label="Customer *">
          <select name="cust_id" value={form.cust_id} onChange={handleChange} required className={inputCls}>
            <option value="">-- Select Customer --</option>
            {customers.filter((c) => c.cust_type === 'A' || c.cust_type === 'B').map((c) => (
        ...
        <FieldGroup label="Premium Amount ($) *">
          <input type="number" step="0.01" name="premium_amount" value={form.premium_amount} onChange={handleChange} required placeholder="1200.00" className={inputCls} />
        </FieldGroup>
        <FieldGroup label="Status *">
          <select name="a_policy_status" value={form.a_policy_status} onChange={handleChange} required className={inputCls}>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 rounded-xl bg-white p-6 shadow-2xl w-80">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Delete Auto Policy</h3>
            <p className="text-sm text-gray-600 mb-5">Delete policy <strong>#{deleteTarget.a_policy_id}</strong>? This cannot be undone.</p>
